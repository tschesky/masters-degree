...
instance Monad RWSP where
  ...
  m >>= f = RWSP (\r s -> let (val, write, state) = runRWSP m r s
                         in let (val1, write1, state1) = runRWSP (f val) r state
                            in (val1, write `mappend` write1, state1))
...
instance Monad RWSE where
  ...
  m >>= f = RWSE (\r s0 -> ((runRWSE m r s0) >>= (\(a, w, s1) -> (runRWSE (f a) r s1) >>= (\(a', w', s2) -> Right (a', w `mappend` w', s2) ) ) ) )
